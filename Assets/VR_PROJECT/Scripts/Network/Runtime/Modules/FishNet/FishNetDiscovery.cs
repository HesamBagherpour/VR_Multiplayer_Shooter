...
namespace VR_PROJECT.Network.Modules.FishNet
{
	...
	/// </summary>
	public sealed class FishNetDiscovery : MonoBehaviour
	{
		/// <summary>
		...
		{
			if (_networkManager.CanLog(LoggingType.Common)) Debug.Log($"[{nameof(FishNetDiscovery)}] {message}", this);
		}
		...
		{
			if (_networkManager.CanLog(LoggingType.Warning)) Debug.LogWarning($"[{nameof(FishNetDiscovery)}] {message}", this);
		}
		...
		{
			if (_networkManager.CanLog(LoggingType.Error)) Debug.LogError($"[{nameof(FishNetDiscovery)}] {message}", this);
		}
	}
}
